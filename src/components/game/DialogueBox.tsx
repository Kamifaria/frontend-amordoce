...
export const DialogueBox: React.FC<DialogueBoxProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Speech text area (Fixo, não rola) */}
      <div className="px-5 pt-5 md:p-5 pb-3 shrink-0">
      ...
      {/* Choice Buttons (Flex-1 e Overflow Auto para scroll independente) */}
      {isChoiceActive && !isTyping && choices && onSelectChoice && (
        <div className="flex-1 overflow-y-auto px-5 pt-3 flex flex-col gap-2.5 pointer-events-auto border-t border-pink-500/15 custom-scrollbar">
          {choices.map((choice, index) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3.5 rounded-xl border text-xs md:text-sm font-semibold flex justify-between items-start gap-3 transition-all active:scale-[0.98] shrink-0 ${
                  hasEnoughPA
      ...
      {!isTyping && !isChoiceActive && (
        <div className="px-5 pt-1 flex justify-end items-center gap-1.5 text-pink-400 font-bold text-[9px] uppercase tracking-widest animate-pulse shrink-0">
          Avançar
